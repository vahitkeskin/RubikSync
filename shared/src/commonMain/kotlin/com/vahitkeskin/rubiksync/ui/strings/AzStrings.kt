package com.vahitkeskin.rubiksync.ui.strings

val AzStrings = AppStrings(
    appTitle = "RUBİK SİNK",
    appSubtitle = "3D İNTERAKTİV SİMULATOR",
    splashSubtitle = "3D İNTERAKTİV SİMULATOR VƏ HƏLL EDİCİ",
    settingsTitle = "Ayarlar",
    settingsSubtitle = "Tətbiq tərcihlərini fərdiləşdirin",
    themeMode = "Mövzu Rejimi",
    themeSubtitle = "Tətbiqin görünüşünü tənzimləyin",
    themeLight = "Açıq",
    themeDark = "Qapalı",
    themeSystem = "Sistem",
    languageTitle = "Dil Seçimi",
    languageSubtitle = "19 fərqli dil ilə dil dəstəyi",
    aboutTitle = "Haqqında",
    aboutSubtitle = "Tətbiq məlumatları",
    versionTitle = "Versiya",
    platformTitle = "Qurucu",
    copyright = "RUBIK SYNC © ",
    scrambleButton = "🎲 Qarışdır",
    undoButton = "↩️ Geri Al",
    resetButton = "🔄 Sıfırla",
    designButton = "🎨 Dizayn Et",
    solveButton = "🧠 Həll Et",
    speedLabel = "⏱ Sürət",
    solutionTitle = "🧩 Həll: ",
    solutionComplete = "Həll Tamamlandı! 🎉",
    progressLabel = " tamamlandı",
    backToStart = "⏮ Əvvələ",
    nextStep = "İrəli ⏭",
    technicalDetails = "📖 Texniki Həll Detalları",
    phaseLabel = "Mərhələ: ",
    nextMoveLabel = "Növbəti Hərəkət: ",
    remainingLabel = "Qalan: %s hərəkət",
    mathFormulaTitle = "🔢 Riyazi Formul və Rotasiya",
    solvedSuccessTitle = "Həll Tamamlandı! 🎉",
    solvedSuccessDesc = "Zəka kubu uğurla həll edilmiş vəziyyətə gətirildi.",
    scannerTitle = "📸 Kub Skaneri",
    facesScanned = " üz skan olundu",
    noImage = "Şəkil Yoxdur",
    alignGridDesc = "🔧 Toru kubun kənarlarına uyğunlaşdırın",
    sizeLabel = "Ölçü",
    horizontalLabel = "Üfüqi",
    verticalLabel = "Şaquli",
    colorPreview = "🎨 Rəng Önizləməsi (düzəltmək üçün klikləyin)",
    retakePhoto = "📸 Yenidən Çək",
    cancelButton =  "İmtina",
    backButton =  "Geri",
    nextButton =  "İrəli",
    setButton =  "Tənzimlə",
    cubeMapTitle = "KUB XƏRİTƏSİ",
    editorTitle = "🎨 Kub Redaktoru",
    editorSubtitle = "Rəngləri boyayın və ya kamera ilə skan edin",
    cameraScanTitle = "📸 Kamera ilə Skan",
    cameraScanSubtitle = "6 üzü avtomatik müəyyən et",
    brushColorPrefix = "🖌 Boyaq Rəngi: ",
    clearButton = "🗑 Təmizlə",
    applyButton = "✓ Tətbiq Et",
    pasteJsonDesc = "Kub JSON vəziyyətini yapışdırın:",
    importJsonButton = "✓ İdxal Et",
    errorScanAllFaces = "Zəhmət olmasa bütün üzləri skan edin.",
    successScanComplete = "Kub uğurla skan edildi! ✅",
    errorPhotoResolution = "Şəkil analiz edilə bilmədi.",
    themeOptionLightDesc = "Aydınlıq Görünüş",
    themeOptionDarkDesc = "Qaranlıq Görünüş",
    themeOptionSystemDesc = "Sistem Varsayılanı",
    cancelAction = "İptal",
    importJsonTitle = "JSON İdxal Et",
    importAction = "İdxal Et",
    movesLabel = "Hərəkət",
    statusLabel = "Status",
    solvedStatus = "Həll edilib",
    scrambledStatus = "Qarışıq",
    tabMoves = "🎮 Hərəkətlər",
    tabActions = "⚡ Əməliyyatlar",
    tabAI = "🧠 Süni İntellekt",
    scanAction = "Skan et",
    faceU = "Üst",
    faceD = "Alt",
    faceL = "Sol",
    faceR = "Sağ",
    faceF = "Ön",
    faceB = "Arxa",
    colorOrange = "Narıncı",
    colorRed = "Qırmızı",
    colorYellow = "Sarı",
    colorWhite = "Ağ",
    colorGreen = "Yaşıl",
    colorBlue = "Mavi",
    cubeStateApplied = "Küp vəziyyəti tətbiq olundu! ✅",
    invalidCubeDesign = "Keçərsiz küp dizaynı!",
    jsonImportSuccess = "JSON uğurla idxal edildi!",
    jsonImportError = "Yanlış JSON formatı!",
    faceDetectedMessage = "%s: %s üzü aşkar edildi",
    faceNotDetected = "Üz aşkar edilmədi!",
    faceTitleLabel = "%s (%s) Üzü",
    centerColorLabel = "%s Mərkəzi",
    guideU = "Narıncı mərkəz üstdə olsun. Yaşıl üz sizə baxsın, ağ üz sağınızda qalsın.",
    guideD = "Qırmızı mərkəz üstdə olsun. Yaşıl üz sizə baxsın, ağ üz sağınızda qalsın.",
    guideL = "Sarı mərkəz üstdə olsun. Narıncı üz üstdə qalsın, yaşıl üz sağınızda qalsın.",
    guideR = "Ağ mərkəz üstdə olsun. Narıncı üz üstdə qalsın, mavi üz sağınızda qalsın.",
    guideF = "Yaşıl mərkəz üstdə olsun. Narıncı üz üstdə qalsın, ağ üz sağınızda qalsın.",
    guideB = "Mavi mərkəz üstdə olsun. Narıncı üz üstdə qalsın, sarı üz sağınızda qalsın.",
    playbackPause = "⏸ Dayandır",
    playbackPlay = "▶ Oynat",
    cubeAlreadySolved = "Küp artıq həll edilib! ✅",
    solutionNotFound = "Həll tapılmadı!",
    solutionFound = "%s addımda həll tapıldı!",
    scanGuidanceFace = "Zəhmət olmasa %s (%s) üzünü skan edin.",
    scanGuidanceAllScanned = "Bütün üzlər skan edildi! 'Quraşdır' düyməsini basa bilərsiniz.",
    scanGuidanceRemaining = "Bu üz skan edilib. Zəhmət olmasa qalan üzləri skan edin: %s",
    takePhotoLabel =  "Şəkil çək",
    chooseGalleryLabel =  "Qalereyadan seç",
    selectImageLabel =  "Şəkil seç",
    showcaseWelcomeText = "RubikSync-ə xoş gəlmisiniz! Tətbiq xüsusiyyətlərini kəşf edək.",
    showcaseEditableText = "Kub redaktəsini aktiv/deaktiv etmək üçün kilid ikonuna toxunun.",
    showcaseShakeToScrambleText = "Telefonu silkələyərək qarışdırma funksiyasını aktivləşdirmək və ya söndürmək üçün bu ikonaya toxunun.",
    showcaseSoundText = "Dönmə səsini açmaq/söndürmək üçün səs ikonuna toxunun.",
    showcaseSettingsText = "Tema və dil dəyişdirmək üçün parametrləri açın.",
    showcaseInteractiveCubeText = "Küpü sərbəst döndərmək üçün iki barmağınızı, müəyyən sətir və ya sütunları çevirmək üçün isə tək barmağınızı istifadə edə bilərsiniz.",
    showcaseMovesText = "Bu paneldən kubun qatlarını döndürə bilərsiniz.",
    showcaseActionsText = "Use this panel to scramble, undo moves, or reset the cube.",
    showcaseSolveText = "Süni intellekt tabından kubunuzu avtomatik həll edə bilərsiniz.",
    showcaseScrambleText = "Bu düyməni istifadə edərək kubu təsadüfi qarışdıra bilərsiniz.",
    showcaseUndoText = "Bu düyməni istifadə edərək son hərəkətinizi geri ala bilərsiniz.",
    showcaseResetText = "Kubu həll olunmuş başlanğıc vəziyyətinə sıfırlaya bilərsiniz.",
    showcaseDesignText = "Üzləri əllə rəngləyərək və ya kamera vasitəsilə skan edərək kubu dizayn edə bilərsiniz.",
    showcaseEditorFaces = "Kubun hansı üzünü redaktə etmək istədiyinizi seçmək üçün bu tablardan istifadə edin.",
    showcaseEditorGrid = "Seçdiyiniz rənglə kubun stikerlərini rəngləmək üçün bu kvadratlara klikləyin.",
    showcaseEditorPalette = "Rəngləmək istədiyiniz rəngi bu palitradan seçə bilərsiniz.",
    showcaseEditorApply = "Rənglənmiş rəng sxemini kuba tətbiq etmək üçün bu düyməyə klikləyin.",
    showcaseEditorScan = "Şəkil çəkərək rəng vəziyyətlərini avtomatik təyin etmək üçün skaner sehrbazını başlada bilərsiniz.",
    showcaseScannerIndicator = "Skan ediləcək bütün kub üzlərini və skan statusunu buradan izləyə bilərsiniz.",
    showcaseScannerGuide = "Şəkil çəkərkən kubu bu təlimatda göstərilən istiqamətdə saxladığınızdan əmin olun.",
    showcaseScannerCapture = "Kubun müvafiq üzünün şəklini çəkmək və ya qalereyadan yükləmək üçün bura klikləyin.",
    showcaseScannerSliders = "Yaşıl bələdçi xətlərini kubun kvadratları ilə hizalamaq üçün bu sürgülərdən istifadə edin.",
    showcaseScannerPreview = "Təsnif edilmiş rəngləri buradan yoxlayın. Səhv müəyyən edilmiş kvadrat varsa, düzəltmək üçün üzərinə klikləyin.",
    showcaseScannerAction = "Növbəti üzə keçmək və ya skan etməyi tamamlamaq üçün bu naviqasiya düymələrindən istifadə edin.",
    skipShowcase = "Tanıtımı Keç",
    bestTimesTitle = "Ən yaxşı vaxtlar",
    movesCountLabel = "%s gediş",
    scanGuidanceExampleU = "Kubu masanın üstünə qoyun, narıncı üz yuxarı baxsın",
    scanGuidanceExampleD = "Kubu tərsinə çevirin, qırmızı üz yuxarı baxsın",
    scanGuidanceExampleL = "Kubu sola döndərin, sarı mərkəz sizə baxsın",
    scanGuidanceExampleR = "Kubu sağa döndərin, ağ mərkəz sizə baxsın",
    scanGuidanceExampleF = "Kubu düz tutun, yaşıl mərkəz sizə baxsın",
    scanGuidanceExampleB = "Kubu 180° çevirin, mavi mərkəz sizə baxsın",
    showReadmeButton = "Oxu 📖",
    readmeScreenTitle = "İstifadəçi Təlimatı",
    permissionRequiredTitle = "Kamera İcazəsi Lazımdır",
    cameraPermissionDesc = "Zəka kubunun rənglərini avtomatik müəyyən etmək üçün kameranızı istifadə etməliyik. Kameranızla kubun üzlərini çəkərək saniyələr içində həlli görə bilərsiniz. İcazəniz təhlükəsiz şəkildə saxlanılır.",
    permissionLater = "Daha Sonra",
    permissionGrant = "İcazə Ver",
    cameraPermissionDenied = "Kamera icazəsi rədd edildi. Skan etmək üçün qalereyadan istifadə edə bilərsiniz.",
    cubeSkinTitle = "Küp Dizaynı",
    cubeSkinSubtitle = "3D kubunuz üçün görünüş seçin",
    skinClassic = "Klassik",
    skinShinyGold = "Parlaq Qızıl",
    skinShinySilver = "Parlaq Gümüş",
    skinNeon = "Neon Parıltısı",
    skinPastel = "Pastel Röyası",
    skinClassicDesc = "Standart görünüş",
    skinShinyGoldDesc = "Lüks qızıl metalik",
    skinShinySilverDesc = "Futurist xrom görünüşü",
    skinNeonDesc = "Canlı parlayan tonlar",
    skinPastelDesc = "Yumşaq və təmiz tonlar",
    showPreview = "Önizləməni Göstər",
    closePreview = "Önizləməni Bağla",
)
