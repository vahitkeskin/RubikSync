package com.vahitkeskin.rubiksync.ui.strings

val PlStrings = AppStrings(
    appTitle = "RUBIK SYNC",
    appSubtitle = "INTERAKTYWNY SYMULATOR 3D",
    splashSubtitle = "INTERAKTYWNY SYMULATOR I SOLVER 3D",
    settingsTitle = "Ustawienia",
    settingsSubtitle = "Dostosuj preferencje aplikacji",
    themeMode = "Tryb motywu",
    themeSubtitle = "Dostosuj wygląd aplikacji",
    themeLight = "Jasny",
    themeDark = "Ciemny",
    themeSystem = "Systemowy",
    languageTitle = "Język",
    languageSubtitle = "Wsparcie dla 19 różnych języków",
    aboutTitle = "O aplikacji",
    aboutSubtitle = "Informacje o aplikacji",
    versionTitle = "Wersja",
    platformTitle = "Założyciel",
    copyright = "RUBIK SYNC © ",
    scrambleButton = "🎲 Pomieszaj",
    undoButton = "↩️ Cofnij",
    resetButton = "🔄 Resetuj",
    designButton = "🎨 Zaprojektuj",
    solveButton = "🧠 Rozwiąż",
    speedLabel = "⏱ Szybkość",
    solutionTitle = "🧩 Rozwiązanie: ",
    solutionComplete = "Rozwiązanie ukończone! 🎉",
    progressLabel = " ukończono",
    backToStart = "⏮ Od początku",
    nextStep = "Dalej ⏭",
    technicalDetails = "📖 Szczegóły Rozwiązania",
    phaseLabel = "Faza: ",
    nextMoveLabel = "Następny ruch: ",
    remainingLabel = "Pozostało: %s ruchów",
    mathFormulaTitle = "🔢 Wzór matematyczny i rotacja",
    solvedSuccessTitle = "Rozwiązanie ukończone! 🎉",
    solvedSuccessDesc = "Kostka Rubika została pomyślnie ułożona.",
    scannerTitle = "📸 Skaner Kostki",
    facesScanned = " ścianek zeskanowano",
    noImage = "Brak obrazu",
    alignGridDesc = "🔧 Dopasuj siatkę do krawędzi kostki",
    sizeLabel = "Rozmiar",
    horizontalLabel = "Poziomo",
    verticalLabel = "Pionowo",
    colorPreview = "🎨 Podgląd kolorów (kliknij, aby poprawić)",
    retakePhoto = "📸 Powtórz",
    cancelButton =  "Anuluj",
    backButton =  "Wróć",
    nextButton =  "Dalej",
    setButton =  "Ustaw",
    cubeMapTitle = "MAPA KOSTKI",
    editorTitle = "🎨 Edytor Kostki",
    editorSubtitle = "Pomaluj ścianki lub zeskanuj aparatem",
    cameraScanTitle = "📸 Skanowanie aparatem",
    cameraScanSubtitle = "Automatycznie wykryj 6 ścianek",
    brushColorPrefix = "🖌 Kolor pędzla: ",
    clearButton = "🗑 Wyczyść",
    applyButton = "✓ Zastosuj",
    pasteJsonDesc = "Wklej stan JSON kostki:",
    importJsonButton = "✓ Importuj",
    errorScanAllFaces = "Zeskanuj wszystkie ścianki.",
    successScanComplete = "Kostka została pomyślnie zeskanowana! ✅",
    errorPhotoResolution = "Nie udało się przetworzyć zdjęcia.",
    themeOptionLightDesc = "Jasny Wygląd",
    themeOptionDarkDesc = "Ciemny Wygląd",
    themeOptionSystemDesc = "Domyślny systemowy",
    cancelAction = "Anuluj",
    importJsonTitle = "Importuj JSON",
    importAction = "Importuj",
    movesLabel = "Ruchy",
    statusLabel = "Status",
    solvedStatus = "Rozwiązany",
    scrambledStatus = "Pomieszany",
    tabMoves = "🎮 Ruchy",
    tabActions = "⚡ Akcje",
    tabAI = "🧠 AI",
    scanAction = "Skanuj",
    faceU = "Góra",
    faceD = "Dół",
    faceL = "Lewo",
    faceR = "Prawo",
    faceF = "Przód",
    faceB = "Tył",
    colorOrange = "Pomarańczowy",
    colorRed = "Czerwony",
    colorYellow = "Żółty",
    colorWhite = "Biały",
    colorGreen = "Zielony",
    colorBlue = "Niebieski",
    cubeStateApplied = "Stan kostki zastosowany! ✅",
    invalidCubeDesign = "Nieprawidłowy projekt kostki!",
    jsonImportSuccess = "JSON zaimportowany pomyślnie!",
    jsonImportError = "Nieprawidłowy format JSON!",
    faceDetectedMessage = "%s: wykryto ścianę %s",
    faceNotDetected = "Ściana nie została wykryta!",
    faceTitleLabel = "Ściana %s (%s)",
    centerColorLabel = "Środek %s",
    guideU = "Pomarańczowy środek na górze. Zielona ściana skierowana do Ciebie, biała ściana po prawej.",
    guideD = "Czerwony środek na górze. Zielona ściana skierowana do Ciebie, biała ściana po prawej.",
    guideL = "Żółty środek na górze. Pomarańczowa ściana na górze, zielona ściana po prawej.",
    guideR = "Biały środek na górze. Pomarańczowa ściana na górze, niebieska ściana po prawej.",
    guideF = "Zielony środek na górze. Pomarańczowa ściana na górze, biała ściana po prawej.",
    guideB = "Niebieski środek na górze. Pomarańczowa ściana na górze, żółta ściana po prawej.",
    playbackPause = "⏸ Pauza",
    playbackPlay = "▶ Odtwórz",
    cubeAlreadySolved = "Kostka jest już ułożona! ✅",
    solutionNotFound = "Nie znaleziono rozwiązania!",
    solutionFound = "Rozwiązanie znalezione w %s ruchach!",
    scanGuidanceFace = "Proszę zeskanować ścianę %s (%s).",
    scanGuidanceAllScanned = "Wszystkie ściany zeskanowane! Możesz nacisnąć przycisk 'Ustaw'.",
    scanGuidanceRemaining = "Ta ściana jest zeskanowana. Zeskanuj pozostałe ściany: %s",
    takePhotoLabel =  "Zrób zdjęcie",
    chooseGalleryLabel =  "Wybierz z galerii",
    selectImageLabel =  "Wybierz obraz",
    showcaseWelcomeText = "Witamy w RubikSync! Odkryjmy funkcje aplikacji.",
    showcaseEditableText = "Dotknij ikony zamka, aby włączyć/wyłączyć edycję kostki.",
    showcaseShakeToScrambleText = "Stuknij tę ikonę, aby włączyć lub wyłączyć funkcję potrząsania w celu pomieszania.",
    showcaseSoundText = "Dotknij ikony dźwięku, aby włączyć/wyłączyć dźwięk obrotu.",
    showcaseSettingsText = "Otwórz ustawienia, aby zmienić motyw i język.",
    showcaseInteractiveCubeText = "Obracaj cały sześcian dwoma palcami lub obracaj określone wiersze i kolumny jednym palcem.",
    showcaseMovesText = "Użyj tego panelu, aby obracać warstwy kostki.",
    showcaseActionsText = "Use this panel to scramble, undo moves, or reset the cube.",
    showcaseSolveText = "Użyj zakładki AI, aby automatycznie rozwiązać kostkę.",
    showcaseScrambleText = "Możesz losowo pomieszać kostkę za pomocą tego przycisku.",
    showcaseUndoText = "Możesz cofnąć swój ostatni ruch za pomocą tego przycisku.",
    showcaseResetText = "Możesz zresetować kostkę do jej ułożonego stanu początkowego.",
    showcaseDesignText = "Możesz zaprojektować kostkę, malując ścianki ręcznie lub skanując je aparatem.",
    showcaseEditorFaces = "Użyj tych zakładek, aby wybrać, którą ścianę kostki chcesz edytować.",
    showcaseEditorGrid = "Klikaj te kwadraty, aby pomalować naklejki kostki wybranym kolorem.",
    showcaseEditorPalette = "Wybierz kolor z tej palety, którym chcesz malować.",
    showcaseEditorApply = "Kliknij ten przycisk, aby zastosować pomalowany schemat kolorów do kostki.",
    showcaseEditorScan = "Możesz uruchomić kreatora skanera, aby automatycznie wykryć kolory na podstawie zdjęć.",
    showcaseScannerIndicator = "Śledź stąd wszystkie ściany kostki do zeskanowania oraz status ich skanowania.",
    showcaseScannerGuide = "Upewnij się, że trzymasz kostkę w orientacji pokazanej w tym przewodniku podczas robienia zdjęć.",
    showcaseScannerCapture = "Kliknij tutaj, aby zrobić zdjęcie odpowiedniej ściany kostki lub wczytać je z galerii.",
    showcaseScannerSliders = "Użyje tych suwaków, aby wyrównać zielone linie pomocnicze z kwadratami kostki.",
    showcaseScannerPreview = "Sprawdź tutaj rozpoznane kolory. Jeśli kwadrat został błędnie zidentyfikowany, kliknij go, aby to poprawić.",
    showcaseScannerAction = "Użyj tych przycisków nawigacyjnych, aby przejść do następnej ściany lub zakończyć skanowanie.",
    skipShowcase = "Pomiń samouczek",
    bestTimesTitle = "Najlepsze czasy",
    movesCountLabel = "%s ruchów",
    scanGuidanceExampleU = "Połóż kostkę na stole, pomarańczowa ściana do góry",
    scanGuidanceExampleD = "Odwróć kostkę, czerwona ściana do góry",
    scanGuidanceExampleL = "Obróć kostkę w lewo, żółty środek w Twoją stronę",
    scanGuidanceExampleR = "Obróć kostkę w prawo, biały środek w Twoją stronę",
    scanGuidanceExampleF = "Trzymaj kostkę prosto, zielony środek w Twoją stronę",
    scanGuidanceExampleB = "Obróć kostkę o 180°, niebieski środek w Twoją stronę",
    showReadmeButton = "Czytaj 📖",
    readmeScreenTitle = "Instrukcja obsługi",
    permissionRequiredTitle = "Wymagany Dostęp do Aparatu",
    cameraPermissionDesc = "Musimy użyć Twojego aparatu, aby automatycznie wykryć kolory kostki Rubika. Robiąc zdjęcia ścianom kostki za pomocą aparatu, zobaczysz rozwiązanie w kilka sekund. Twoja zgoda jest bezpiecznie przechowywana.",
    permissionLater = "Później",
    permissionGrant = "Zezwól",
    cameraPermissionDenied = "Dostęp do aparatu zablokowany. Możesz użyć galerii do skanowania.",
    cubeSkinTitle = "Wygląd Kostki",
    cubeSkinSubtitle = "Wybierz wygląd swojej kostki 3D",
    skinClassic = "Klasyczny",
    skinShinyGold = "Błyszczące Złoto",
    skinShinySilver = "Błyszczące Srebro",
    skinNeon = "Neony",
    skinPastel = "Pastelowy Sen",
    skinClassicDesc = "Domyślny wygląd",
    skinShinyGoldDesc = "Luksusowy złoty metalik",
    skinShinySilverDesc = "Futurystyczny chromowany wygląd",
    skinNeonDesc = "Żywe, świecące odcienie",
    skinPastelDesc = "Delikatne i czyste tony",
    showPreview = "Pokaż podgląd",
    closePreview = "Zamknij podgląd",
)
