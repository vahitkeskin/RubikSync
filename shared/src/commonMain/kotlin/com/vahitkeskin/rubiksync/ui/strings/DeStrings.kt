package com.vahitkeskin.rubiksync.ui.strings

val DeStrings = AppStrings(
    appTitle = "RUBIK SYNC",
    appSubtitle = "3D INTERAKTIVER SIMULATOR",
    splashSubtitle = "3D INTERAKTIVER SIMULATOR & LÖSER",
    settingsTitle = "Einstellungen",
    settingsSubtitle = "Anwendungseinstellungen anpassen",
    themeMode = "Design-Modus",
    themeSubtitle = "Aussehen der Anwendung anpassen",
    themeLight = "Hell",
    themeDark = "Dunkel",
    themeSystem = "System",
    languageTitle = "Sprache",
    languageSubtitle = "Sprachunterstützung mit 19 verschiedenen Sprachen",
    aboutTitle = "Über",
    aboutSubtitle = "Anwendungsinformationen",
    versionTitle = "Version",
    platformTitle = "Gründer",
    copyright = "RUBIK SYNC © ",
    scrambleButton = "🎲 Mischen",
    undoButton = "↩️ Rückgängig",
    resetButton = "🔄 Zurücksetzen",
    designButton = "🎨 Entwerfen",
    solveButton = "🧠 Lösen",
    speedLabel = "⏱ Tempo",
    solutionTitle = "🧩 Lösung: ",
    solutionComplete = "Lösung abgeschlossen! 🎉",
    progressLabel = " abgeschlossen",
    backToStart = "⏮ Neustart",
    nextStep = "Weiter ⏭",
    technicalDetails = "📖 Technische Lösungsdetails",
    phaseLabel = "Phase: ",
    nextMoveLabel = "Nächster Zug: ",
    remainingLabel = "Verbleibend: %s Züge",
    mathFormulaTitle = "🔢 Mathematische Formel & Rotation",
    solvedSuccessTitle = "Lösung abgeschlossen! 🎉",
    solvedSuccessDesc = "Der Zauberwürfel wurde erfolgreich gelöst.",
    scannerTitle = "📸 Würfel-Scanner",
    facesScanned = " Seiten gescannt",
    noImage = "Kein Bild",
    alignGridDesc = "🔧 Raster an den Würfelkanten ausrichten",
    sizeLabel = "Größe",
    horizontalLabel = "Horizontal",
    verticalLabel = "Vertikal",
    colorPreview = "🎨 Farbvorschau (Klicken zum Ändern)",
    retakePhoto = "📸 Erneut aufnehmen",
    cancelButton =  "Abbrechen",
    backButton =  "Zurück",
    nextButton =  "Weiter",
    setButton =  "Festlegen",
    cubeMapTitle = "WÜRFELPLAN",
    editorTitle = "🎨 Würfel-Editor",
    editorSubtitle = "Farben malen oder mit Kamera scannen",
    cameraScanTitle = "📸 Kamera-Scan",
    cameraScanSubtitle = "6 Seiten automatisch erkennen",
    brushColorPrefix = "🖌 Malfarbe: ",
    clearButton = "🗑 Löschen",
    applyButton = "✓ Anwenden",
    pasteJsonDesc = "Würfel-JSON-Zustand einfügen:",
    importJsonButton = "✓ Importieren",
    errorScanAllFaces = "Bitte scannen Sie alle Seiten.",
    successScanComplete = "Würfel erfolgreich gescannt! ✅",
    errorPhotoResolution = "Foto konnte nicht aufgelöst werden.",
    themeOptionLightDesc = "Helles Design",
    themeOptionDarkDesc = "Dunkles Design",
    themeOptionSystemDesc = "Systemstandard",
    cancelAction = "Abbrechen",
    importJsonTitle = "JSON importieren",
    importAction = "Importieren",
    movesLabel = "Züge",
    statusLabel = "Status",
    solvedStatus = "Gelöst",
    scrambledStatus = "Gemischt",
    tabMoves = "🎮 Züge",
    tabActions = "⚡ Aktionen",
    tabAI = "🧠 KI",
    scanAction = "Scannen",
    faceU = "Oben",
    faceD = "Unten",
    faceL = "Links",
    faceR = "Rechts",
    faceF = "Vorne",
    faceB = "Hinten",
    colorOrange = "Orange",
    colorRed = "Rot",
    colorYellow = "Gelb",
    colorWhite = "Weiß",
    colorGreen = "Grün",
    colorBlue = "Blau",
    cubeStateApplied = "Würfelzustand angewendet! ✅",
    invalidCubeDesign = "Ungültiges Würfeldesign!",
    jsonImportSuccess = "JSON erfolgreich importiert!",
    jsonImportError = "Ungültiges JSON-Format!",
    faceDetectedMessage = "%s: %s Seite erkannt",
    faceNotDetected = "Seite nicht erkannt!",
    faceTitleLabel = "%s (%s) Seite",
    centerColorLabel = "%s-Zentrum",
    guideU = "Orangefarbene Mitte oben. Grüne Seite zeigt zu Ihnen, weiße Seite rechts.",
    guideD = "Rote Mitte oben. Grüne Seite zeigt zu Ihnen, weiße Seite rechts.",
    guideL = "Gelbe Mitte oben. Orangefarbene Seite oben, grüne Seite rechts.",
    guideR = "Weiße Mitte oben. Orangefarbene Seite oben, blaue Seite rechts.",
    guideF = "Grüne Mitte oben. Orangefarbene Seite oben, weiße Seite rechts.",
    guideB = "Blaue Mitte oben. Orangefarbene Seite oben, gelbe Seite rechts.",
    playbackPause = "⏸ Pause",
    playbackPlay = "▶ Abspielen",
    cubeAlreadySolved = "Würfel bereits gelöst! ✅",
    solutionNotFound = "Keine Lösung gefunden!",
    solutionFound = "Lösung in %s Schritten gefunden!",
    scanGuidanceFace = "Bitte scannen Sie die Seite %s (%s).",
    scanGuidanceAllScanned = "Alle Seiten gescannt! Sie können auf die Schaltfläche 'Einstellen' klicken.",
    scanGuidanceRemaining = "Diese Seite wurde gescannt. Bitte scannen Sie die verbleibenden Seiten: %s",
    takePhotoLabel =  "Foto aufnehmen",
    chooseGalleryLabel =  "Aus Galerie wählen",
    selectImageLabel =  "Bild auswählen",
    showcaseWelcomeText = "Willkommen bei RubikSync! Entdecken Sie die App-Funktionen.",
    showcaseEditableText = "Tippen Sie auf das Schloss-Symbol, um die Würfelbearbeitung zu aktivieren/deaktivieren.",
    showcaseShakeToScrambleText = "Tippen Sie auf dieses Symbol, um die Schüttel-Mischfunktion zu aktivieren oder zu deaktivieren.",
    showcaseSoundText = "Tippen Sie auf das Lautsprecher-Symbol, um den Drehsound ein-/auszuschalten.",
    showcaseSettingsText = "Öffnen Sie die Einstellungen, um Design und Sprache zu ändern.",
    showcaseInteractiveCubeText = "Drehen Sie den gesamten Würfel mit zwei Fingern oder drehen Sie bestimmte Zeilen und Spalten mit einem Finger.",
    showcaseMovesText = "Verwenden Sie dieses Panel, um die Würfelschichten zu drehen.",
    showcaseActionsText = "Use this panel to scramble, undo moves, or reset the cube.",
    showcaseSolveText = "Verwenden den KI-Tab, um Ihren Würfel automatisch zu lösen.",
    showcaseScrambleText = "Mit dieser Schaltfläche können Sie den Würfel zufällig mischen.",
    showcaseUndoText = "Mit dieser Schaltfläche können Sie Ihren letzten Zug rückgängig machen.",
    showcaseResetText = "Sie können den Würfel in seinen gelösten Ausgangszustand zurücksetzen.",
    showcaseDesignText = "Sie können den Würfel entwerfen, indem Sie die Seiten manuell bemalen oder mit der Kamera scannen.",
    showcaseEditorFaces = "Verwenden Sie diese Registerkarten, um auszuwählen, welche Würfelseite Sie bearbeiten möchten.",
    showcaseEditorGrid = "Klicken Sie auf diese Quadrate, um die Würfelaufkleber mit Ihrer ausgewählten Farbe zu bemalen.",
    showcaseEditorPalette = "Wählen Sie die gewünschte Farbe aus dieser Palette aus.",
    showcaseEditorApply = "Klicken Sie auf diese Schaltfläche, um das gemalte Farbschema auf den Würfel anzuwenden.",
    showcaseEditorScan = "Sie können den Scanner-Assistenten starten, um Farbzustände durch Fotografieren automatisch zu erkennen.",
    showcaseScannerIndicator = "Verfolgen Sie hier alle zu scannenden Würfelseiten und deren Scan-Status.",
    showcaseScannerGuide = "Stellen Sie sicher, dass Sie den Würfel beim Fotografieren in der in dieser Anleitung gezeigten Ausrichtung halten.",
    showcaseScannerCapture = "Klicken Sie hier, um ein Foto der entsprechenden Würfelseite aufzunehmen oder aus der Galerie zu laden.",
    showcaseScannerSliders = "Verwenden Sie diese Schieberegler, um die grünen Führungslinien an den Quadraten des Würfels auszurichten.",
    showcaseScannerPreview = "Überprüfen Sie hier die klassifizierten Farben. Wenn ein Quadrat falsch identifiziert wurde, klicken Sie darauf, um es zu korrigieren.",
    showcaseScannerAction = "Verwenden Sie diese Navigationstasten, um zur nächsten Seite zu gelangen oder den Scan abzuschließen.",
    skipShowcase = "Anleitung überspringen",
    bestTimesTitle = "Beste Zeiten",
    movesCountLabel = "%s Züge",
    scanGuidanceExampleU = "Legen Sie den Würfel auf den Tisch, orange Seite nach oben",
    scanGuidanceExampleD = "Drehen Sie den Würfel um, rote Seite nach oben",
    scanGuidanceExampleL = "Drehen Sie den Würfel nach links, gelbe Mitte zeigt zu Ihnen",
    scanGuidanceExampleR = "Drehen Sie den Würfel nach rechts, weiße Mitte zeigt zu Ihnen",
    scanGuidanceExampleF = "Halten Sie den Würfel gerade, grüne Mitte zeigt zu Ihnen",
    scanGuidanceExampleB = "Drehen Sie den Würfel um 180°, blaue Mitte zeigt zu Ihnen",
    showReadmeButton = "Lesen 📖",
    readmeScreenTitle = "Benutzerhandbuch",
    permissionRequiredTitle = "Kamerazugriff erforderlich",
    cameraPermissionDesc = "Wir müssen Ihre Kamera verwenden, um die Farben des Zauberwürfels automatisch zu erkennen. Wenn Sie die Seiten des Würfels fotografieren, sehen Sie die Lösung in Sekundenschnelle. Ihre Berechtigung wird sicher gespeichert.",
    permissionLater = "Später",
    permissionGrant = "Erlauben",
    cameraPermissionDenied = "Kamerazugriff verweigert. Sie können die Galerie zum Scannen verwenden.",
    cubeSkinTitle = "Würfel-Design",
    cubeSkinSubtitle = "Wählen Sie ein Design für Ihren 3D-Würfel",
    skinClassic = "Klassisch",
    skinShinyGold = "Glänzendes Gold",
    skinShinySilver = "Glänzendes Silber",
    skinNeon = "Neon-Glühen",
    skinPastel = "Pastell-Traum",
    skinClassicDesc = "Standard-Aussehen",
    skinShinyGoldDesc = "Luxuriöses Gold-Metallic",
    skinShinySilverDesc = "Futuristischer Chrom-Look",
    skinNeonDesc = "Kräftige leuchtende Farben",
    skinPastelDesc = "Sanfte und klare Töne",
    showPreview = "Vorschau anzeigen",
    closePreview = "Vorschau ausblenden",
)
